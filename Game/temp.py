...
def chkVerticalCondition(x , buttonPos , offset , buttonSize ,playerSize , v, a ,b) :
    if a == 1 :
        y = x.bottom
    else :
        y = x.top
    return ((y >= buttonPos[1] - buttonSize[1] - offset and y <= buttonPos[1] - buttonSize[1] * a + offset
             and x.left >= buttonPos[0] - playerSize[0] and x.right <= buttonPos[0] + buttonSize[0] +
             playerSize[0] and v*b > 0))
...
while True:
    ...
    if SCENESHIFT == False:
        ...
        if player_rect.bottom >= 450:
            ...
            VELOCITY = 0
            JUMP = False
        elif chkVerticalCondition(player_rect,BUTTON_POS,VOFFSET,BUTTON_SIZE,PLAYER_SIZE,VELOCITY,1,1):
            player_rect.bottom = BUTTON_POS[1] - BUTTON_SIZE[1]
            VELOCITY = 0
            JUMP = True
            BUTTONPRESS = True
        elif chkVerticalCondition(player_rect,BUTTON_POS,VOFFSET,BUTTON_SIZE,PLAYER_SIZE,VELOCITY,0,-1):
