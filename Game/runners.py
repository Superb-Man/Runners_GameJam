...
player_walk1 = pygame.image.load('Game/Assets/Images/pr1.jpg').convert_alpha()
player_walk1 = pygame.transform.scale(player_walk1, PLAYER_SIZE)
player_walk1.set_colorkey(BLACK)
player_walk2 = pygame.image.load('Game/Assets/Images/pr2.jpg').convert_alpha()
player_walk2 = pygame.transform.scale(player_walk2, PLAYER_SIZE)
player_walk2.set_colorkey(BLACK)
player_walk3 = pygame.image.load('Game/Assets/Images/pr3.jpg').convert_alpha()
player_walk3 = pygame.transform.scale(player_walk3, PLAYER_SIZE)
player_walk3.set_colorkey(BLACK)
players.append(player_walk1)
players.append(player_walk2)
...
player_dead = pygame.image.load('Game/Assets/Images/prd.jpg').convert_alpha()
player_dead = pygame.transform.scale(player_dead, PLAYER_SIZE)
player_dead.set_colorkey(BLACK)
dead_img.append(player_dead)
...
door = pygame.image.load('Game/Assets/Images/door.jpg').convert_alpha()
...
button = pygame.image.load('Game/Assets/Images/b.jpg').convert_alpha()
...
ultakata = pygame.image.load('Game/Assets/Images/uk22.png').convert_alpha()
...
=======
        ...
        state_font = font.render(f'{state + 1}. Ulta-Palta', False, 'white')
        state_font_rect = state_font.get_rect(center=(100, 50))
        screen.blit(state_font, state_font_rect)
    elif state == 5:
        state_font = font.render(f'{state + 1}. Danger', False, 'white')
...
def level7(event):
    global SCENESHIFT, door_rect, BUTTONPRESS, DEAD, DIRECTION, VELOCITY, JUMP, BG, RIGHT, player_rect, SCREEN_WIDTH,JUMPDIRECTION
    global vector
    global inix,iniy,endx,endy
    screen.blit(door, door_rect)
    screen.blit(button, button_rect)
    GRAVITY = 0.1
    ...
    if not DEAD :
        ...
        if event.type == pygame.MOUSEBUTTONUP and vector == False :
            ...
            if endx<=0 :
                endx = 0
            # print(inix,iniy,'initial')
            # print('width',player_rect.left, player_rect.right)
            # print('height', player_rect.bottom,player_rect.top)
        ...
        if vector :
            VELOCITY = 0
            if player_rect.x < endx and RIGHT:
                DIRECTION = RIGHTDIRECTION
                playerAnimation()
                player_rect.x = player_rect.x + SIDEMOVE
                pygame.time.delay(10)
            elif player_rect.x > endx and LEFT:
                DIRECTION = LEFTDIRECTION
                playerAnimation()
<<<<<<< HEAD
                player_rect.left -= SIDEMOVE
...
>>>>>>> 740c92bbe857071881bb0fa0d063d74cd387b8c8
            ...
            if abs(player_rect.x - endx) <= 10 and abs(player_rect.y - endy) <= 50 and vector == True:
                player_rect.x = endx
                player_rect.y = endy
                vector = False
                print('Not entering here')
                VELOCITY += GRAVITY
        if vector == False and JUMP :
            VELOCITY = 5
            player_rect.y-=1
            JUMP = False
...
>>>>>>> 4834d3b29c4debe21ca6e0bea9432d2897f7afd5


def play():
    #global GRAVITY, LEVEL, VELOCITY, SIDEMOVE, SCENESHIFT, OFFSET, VOFFSET, JUMP, LEFT, RIGHT,DEAD, DIRECTION, BUTTONPRESS, BG, BG2
    global SCENESHIFT, door_rect, BUTTONPRESS, DEAD, DIRECTION, VELOCITY, JUMP, BG, RIGHT, player_rect, SCREEN_WIDTH, danger_freq, danger_kata
